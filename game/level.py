        ...
        for r, row in enumerate(self.layout):
            for c, col in enumerate(row):
                if self.layout[r][c] in PLAYER_CHARS:
                    self.players.append(Player(name=PLAYER1_NAME, velocity=5, location=Loc(r, c), parent=self))
                    ...
                    if self.layout[r][c - 1] == Hallway.mapChar and self.layout[r][c + 1] == Hallway.mapChar:
                        self.layout[r] = self.layout[r][:c] + Hallway.mapChar + self.layout[r][c + 1:]
                    elif self.layout[r][c - 1] == Room.mapChar and self.layout[r][c + 1] == Room.mapChar:
                        self.layout[r] = self.layout[r][:c] + Room.mapChar + self.layout[r][c + 1:]
